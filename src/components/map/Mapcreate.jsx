...
// IMPROVEMENT 1: Enhanced MapClickHandler — sets destination on click when active
...
// IMPROVEMENT 1: Reverse geocode a [lat, lng] pair to a human-readable name
...
// IMPROVEMENT 2: Viewer360 — identical to MapCreate's implementation
...
const MapComponent = () => {
    ...
    // IMPROVEMENT 2: 360 viewer state
    ...
    // IMPROVEMENT 1: Map click handler — reverse geocodes and sets destination
    const handleMapClick = useCallback(async (coords) => {
        setDestination(coords);
        setRouteInfo(null);
        setIsSettingDestination(false);
        // Show coords immediately, then replace with reverse-geocoded name
    ...
    const sidebarContent = (
        // IMPROVEMENT 3: Scrollable sidebar — gap and overflow handled per section
        <div className="flex flex-col gap-3 p-3">
            ...
            <Section title="Destino" icon="🏁" open={secOpen.dest} onToggle={() => tog('dest')} accent="red">
                <div className="p-3 flex flex-col gap-2">
                    ...
                    {/* IMPROVEMENT 1: Toggle map-click destination mode */}
            ...
            <Section title={`Rutas del Campus ${dbRoutes.length ? `(${dbRoutes.length})` : ''}`} icon="🚌" open={secOpen.dbRoutes} onToggle={() => tog('dbRoutes')} accent="indigo">
                {/* IMPROVEMENT 3: Scrollable list capped at 240px */}
            ...
            <Section title={`Ubicaciones ${dbPois.length ? `(${dbPois.length})` : ''}`} icon="📌" open={secOpen.pois} onToggle={() => tog('pois')} accent="green">
                {/* IMPROVEMENT 3: Scrollable POI list capped at 320px */}
                    ...
                    {dbPois.map(poi => {
                        ...
                        return (
                            <div key={poi._id} onClick={() => handleSelectPoi(poi)}
                                ...
                                {/* 360 badge in list — consistent with MapCreate */}
                                {poi.imagen && (
                                    <button
    ...
    const mapContent = (
        <div className="relative w-full h-full">
            ...
            {/* IMPROVEMENT 1: Crosshair hint banner when picking destination */}
            ...
            <MapContainer center={mapCenter} zoom={12} maxZoom={22} style={{ height: '100%', width: '100%' }}>
                ...
                {/* IMPROVEMENT 1: MapClickHandler now wired to handleMapClick */}
                ...
                {/* DB route stops */}
                {activeDbRoute && activeDbRoute._stops?.length > 1 && activeDbRoute._stops.map((s, i) => (
                ...
                {activeDbRoute && activeDbRoute._stops?.length > 1 && (
                    <RoutingMachine waypoints={activeDbRoute._stops.map(s => [s.lat, s.lng])} color={activeDbRoute.color || '#6366f1'} />
                )}

                {/* IMPROVEMENT 2: DB POIs — popup includes 360° preview when imagen exists */}
                {dbPois.map(poi => (
                    <Marker key={poi._id} position={[poi.latitud, poi.longitud]} icon={createPoiIcon(poi.categoria)}>
                        <Popup minWidth={220}>
                            <div style={{ minWidth: 200 }}>
                                ...
                                {/* 360° preview block — mirrors MapCreate popup exactly */}
                                {poi.imagen && (
    ...
    /* ─────────────────── LAYOUT ─────────────────── */
    return (
        // IMPROVEMENT 2: Render 360 viewer overlay at root level (above everything)
        <div className="flex flex-col md:flex-row w-full font-sans" style={{ height: 'calc(100vh - 64px)' }}>
            ...
            {/* ── Desktop sidebar ── */}
            {/* IMPROVEMENT 3: sidebar uses overflow-y-auto so the whole panel scrolls */}
