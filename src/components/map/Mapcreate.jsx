...
function parseTxtRoutes(text) {
    ...
    lines.forEach((line, idx) => {
        ...
        if (line.toUpperCase().startsWith('RUTA:')) {
            ...
            if (!nombre) {
                errors.push(`Línea ${idx + 1}: RUTA sin nombre`);
                current = null;
                return;
            }
            ...
            if (/^#[0-9a-fA-F]{3,6}$/.test(color)) {
                ...
                errors.push(`Línea ${idx + 1}: COLOR inválido "${color}", usando default`);
            }
            ...
            const parts = usePipe
                ...
                : raw.split(',').map(p => p.trim());
            if (parts.length < 3) {
                errors.push(`Línea ${idx + 1}: PARADA mal formateada`);
                return;
            }
            const nombre = usePipe
                ? parts[0]
            ...
            if (isNaN(lat) || isNaN(lng)) {
                errors.push(`Línea ${idx + 1}: coordenadas inválidas`);
                return;
            }
            current.stops.push({ nombre, lat, lng, orden: current.stops.length });
        } else {
            if (!line.startsWith('#')) {
                errors.push(`Línea ${idx + 1}: formato no reconocido "${line.slice(0, 40)}..."`);
            }
        }
    });
...
function Viewer360({ imageUrl, onClose }) {
    ...
    useEffect(() => {
        ...
        // Cargar pannellum dinámicamente si no está cargado
        const loadPannellum = () => {
...
// ─── Formulario de POI (con imagen 360) ──────────────────────────────────────
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div style={{
                        ...
                        {['url', 'upload'].map(m => (
                            <button
                                ...
                                style={{
                                    ...
                                    background: imageMode === m ? '#0369a1' : '#e0f2fe',
                                    color: imageMode === m ? '#fff' : '#0369a1',
                                }}
                            >
                                {m === 'url' ? '🔗 URL' : '📂 Archivo'}
                            </button>
                ...
                {imageMode === 'url' ? (
                    ...
                    <div>
                        ...
                        <div
                            ...
                            style={{
                                border: '2px dashed #7dd3fc', borderRadius: 8,
                                padding: '12px', textAlign: 'center', cursor: 'pointer',
                                background: imagePreview ? '#e0f2fe' : '#fff',
                            }}
                        >
                            {imagePreview ? (
                                ...
                                <span style={{ fontSize: 11, color: '#7dd3fc', fontWeight: 600 }}>
                                    📷 Clic para seleccionar imagen equirectangular
                                </span>
                            )}
                        </div>
                    </div>
                    ...
                    <div style={{ position: 'relative' }}>
                        ...
                        <div style={{
                            ...
                            <span style={{ color: '#fff', fontSize: 11, fontWeight: 700 }}>
                                Vista previa — imagen equirectangular
                            </span>
                        </div>
                        <button
                            type="button"
                ...
                <p style={{ fontSize: 10, color: '#6b7280', margin: 0 }}>
                    Usa una imagen equirectangular (proporción 2:1) para mejor experiencia 360°.
                </p>
            </div>
...
function ImportarRutasTxt({ axiosAuth, onImportSuccess, onPreviewRoutes, previewRoutes }) {
    const fileInputRef = useRef(null);
    ...
    const handleFileChange = (e) => {
        ...
        reader.onload = (ev) => {
            ...
            const { rutas, errors } = parseTxtRoutes(text);
            setParsedRoutes(rutas);
            setParseErrors(errors);
            if (rutas.length > 0) {
                setSelectedPreview(0);
                onPreviewRoutes(rutas);
            }
        };
    ...
    const handleImport = async () => {
        ...
        setImporting(true);
        setImportProgress({ actual: 0, total: parsedRoutes.length, nombre: '' });
        ...
        for (let i = 0; i < parsedRoutes.length; i++) {
            ...
            try {
                ...
                console.error(`Error importando ${ruta.nombre}:`, err.response?.data || err.message);
                fallidas++;
            }
        }
        ...
        onImportSuccess(
            ...
            fallidas === 0
                ? `✅ ${exitosas} ruta(s) importadas correctamente`
        ...
        <div style={{
            ...
            <details style={{ fontSize: 11, color: '#374151' }}>
                <summary style={{ cursor: 'pointer', fontWeight: 600, color: '#1e40af' }}>
                    ℹ️ Formato del archivo .txt
                </summary>
                <pre style={{
                    background: '#dbeafe', borderRadius: 6, padding: '8px 10px',
                    fontSize: 10, margin: '6px 0 0', overflowX: 'auto',
                    color: '#1e3a8a', lineHeight: 1.6,
                }}>{`RUTA: Ruta 1
COLOR: #3b82f6
PARADA: Parada A | -0.211 | -78.491
PARADA: Parada B | -0.215 | -78.488`}
                </pre>
            </details>
            ...
            <div
                ...
                style={{
                    ...
                    background: fileName ? '#dbeafe' : '#fff',
            ...
            >
                ...
                {fileName ? (
                    <div>
                        ...
                        <div style={{ fontSize: 12, fontWeight: 600, color: '#1e40af', marginTop: 4 }}>{fileName}</div>
                        <div style={{ fontSize: 11, color: '#6b7280', marginTop: 2 }}>Clic para cambiar archivo</div>
                    </div>
                ) : (
                    <div>
                        <div style={{ fontSize: 24 }}>📂</div>
                        <div style={{ fontSize: 12, fontWeight: 600, color: '#2563eb', marginTop: 4 }}>Arrastra o haz clic para subir</div>
                        <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 2 }}>Solo archivos .txt</div>
                    </div>
                )}
            </div>
            ...
            {parseErrors.length > 0 && (
                <div style={{
                    ...
                    <p style={{ fontSize: 11, fontWeight: 700, color: '#dc2626', margin: '0 0 4px' }}>
                        ⚠️ Advertencias ({parseErrors.length})
                    </p>
                    {parseErrors.map((e, i) => (
                        <div key={i} style={{ fontSize: 10, color: '#b91c1c', lineHeight: 1.5 }}>• {e}</div>
                    ))}
                </div>
            )}
            ...
            {parsedRoutes.length > 0 && (
                <div>
                    <p style={{ fontSize: 11, fontWeight: 700, color: '#15803d', margin: '0 0 6px' }}>
                        ✅ {parsedRoutes.length} ruta(s) listas para importar
                    </p>
                    <p style={{ fontSize: 10, color: '#6b7280', margin: '0 0 6px' }}>
                        Haz clic en una ruta para verla en el mapa
                    </p>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 180, overflowY: 'auto' }}>
                        {parsedRoutes.map((r, idx) => {
                            ...
                            return (
                                <div
                                    key={idx}
                                    onClick={() => handlePreviewSelect(idx)}
                                    style={{
                                        padding: '7px 10px', borderRadius: 8,
                                        border: `1.5px solid ${isSelected ? r.color : '#e5e7eb'}`,
                                        background: isSelected ? r.color + '18' : '#fff',
                                        cursor: 'pointer',
                                ...
                                >
                                    ...
                                    {isSelected && <span style={{ fontSize: 10, color: r.color, fontWeight: 700 }}>👁 Vista</span>}
                                </div>
                            );
                        })}
                    </div>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
                <button
                    ...
                    disabled={importing || parsedRoutes.length === 0}
                    style={{
                        ...btnStyle,
                        background: importing || parsedRoutes.length === 0 ? '#93c5fd' : '#2563eb',
                        flex: 1, fontSize: 12,
                    }}
                >
                    {importing
                        ...
                        : parsedRoutes.length > 0
                            ? `📤 Importar ${parsedRoutes.length} ruta(s)`
                            : '📤 Importar'}
                </button>
                {(parsedRoutes.length > 0 || fileName) && (
                    <button
                        onClick={handleClear}
                        disabled={importing}
                        style={{ ...btnStyle, background: '#6b7280', fontSize: 12, padding: '9px 10px' }}
                    >
                        ✕
                    </button>
                )}
...
const MapCreate = () => {
            ...
            if (editingPoi) {
                await axiosAuth.put(
                    `/admin/mapa/ubicaciones/${editingPoi._id}`,
                    {
                        ...
                        descripcion: poiData.descripcion,
                        categoria:   poiData.categoria,
                        imagen:      poiData.imagen,
                    }
                );
    ...
    const handleSelectRoute = useCallback((r) => {
        const id = r._id || r.id;
        if (selRoute && (selRoute._id || selRoute.id) === id) {
            setSelRoute(null);
            setSelRouteStops([]);
            return;
        }
    ...
    const handleDeleteRoute = async (r, e) => {
        ...
        if (!window.confirm(
            `¿Eliminar la ruta "${r.nombre || r.name}"${stopCount > 0 ? ` y sus ${stopCount} parada(s)` : ''}?`
        )) return;
        setDeletingRoute(id);
        try {
            await Promise.allSettled(
                stops.map(s => {
                    ...
                    return sid ? axiosAuth.delete(`/admin/bus/paradas/${sid}`) : Promise.resolve();
                })
            );
            await axiosAuth.delete(`/admin/bus/rutas/${id}`);
            showMsg('success', `Ruta eliminada${stopCount > 0 ? ` junto con ${stopCount} parada(s)` : ''}`);
            if ((selRoute?._id || selRoute?.id) === id) {
                setSelRoute(null);
                setSelRouteStops([]);
            }
            fetchRoutes();
        } catch (err) {
            const msg = err.response?.status === 401 ? 'Sesión expirada' : 'Error al eliminar la ruta';
            showMsg('error', msg);
        } finally {
    ...
    const handleDeleteAllRoutes = async () => {
        ...
        if (!window.confirm(
            `¿Eliminar las ${savedRoutes.length} rutas${totalStops > 0 ? ` y sus ${totalStops} parada(s)` : ''}? Esta acción no se puede deshacer.`
        )) return;
        setDeletingAll(true);
        let ok = 0, fail = 0;
        ...
        showMsg(fail === 0 ? 'success' : 'error',
            fail === 0 ? `✅ ${ok} ruta(s) y sus paradas eliminadas` : `⚠️ ${ok} eliminadas, ${fail} fallidas`);
        fetchRoutes();
    };
    ...
    const handleUpdateRoute = async () => {
        ...
        try {
            await axiosAuth.put(`/admin/bus/rutas/${editingRoute._id}`, {
                nombre: editingRoute.nombre,
                color:  editingRoute.color,
            });
            showMsg('success', 'Ruta actualizada');
            setEditingRoute(null);
            if (selRoute && (selRoute._id || selRoute.id) === editingRoute._id) {
                setSelRoute(prev => ({ ...prev, nombre: editingRoute.nombre, color: editingRoute.color }));
            }
            fetchRoutes();
        } catch (err) {
            const msg = err.response?.status === 401 ? 'Sesión expirada' : 'Error al actualizar la ruta';
            showMsg('error', msg);
        }
    };
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 64px)', fontFamily: 'system-ui,sans-serif' }}>
            ...
            {viewer360Url && (
                <Viewer360
                    imageUrl={viewer360Url}
                    onClose={() => setViewer360Url(null)}
                />
            )}
                ...
                <div style={{ width: 280, overflowY: 'auto', padding: 12, background: '#fff', borderRight: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', gap: 12, height: '100%' }}>
                    ...
                    {activeTab === 'rutas' && (
                        <>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                                    ...
                                    {savedRoutes.length > 0 && (
                                        <button
                                            ...
                                            style={{ ...btnStyle, padding: '4px 10px', fontSize: 11, background: deletingAll ? '#fca5a5' : '#ef4444' }}
                                        >
                                            {deletingAll ? '⟳' : '🗑️ Todas'}
                                        </button>
                                    )}
                                    ...
                                    <div style={{
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                            ...
                                            <input type="color"
                                                ...
                                                onChange={e => setEditingRoute(prev => ({ ...prev, color: e.target.value }))}
                                                style={{ width: 36, height: 28, border: 'none', cursor: 'pointer' }}
                                            />
                                        </div>
                                        <div style={{ display: 'flex', gap: 6 }}>
                                            <button onClick={handleUpdateRoute}
                                                style={{ ...btnStyle, background: '#1d4ed8', flex: 1, fontSize: 12 }}>
                                                💾 Guardar
                                            </button>
                                            <button onClick={() => setEditingRoute(null)}
                                                style={{ ...btnStyle, background: '#6b7280', fontSize: 12 }}>
                                                ✕
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    return (
                                        <div key={rid} style={{
                                            ...
                                            >
                                                ...
                                                <button
                                                    onClick={e => { e.stopPropagation(); setEditingRoute({ _id: rid, nombre: r.nombre || r.name, color: r.color || '#3b82f6' }); }}
                                                    title="Editar ruta"
                                                    style={{ ...btnStyle, padding: '3px 7px', background: '#3b82f6', fontSize: 10, flexShrink: 0 }}
                                                >✏️</button>
                                                <button
                                                    onClick={e => handleDeleteRoute(r, e)}
                                                    disabled={isDeleting}
                                                    title="Eliminar ruta"
                                                    style={{ ...btnStyle, padding: '3px 7px', background: isDeleting ? '#fca5a5' : '#ef4444', fontSize: 10, flexShrink: 0 }}
                                                >{isDeleting ? '⟳' : '🗑️'}</button>
                                            </div>
                                                    ...
                                                    {selRouteStops.map((s, i) => (
                                                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '2px 0' }}>
                                                            <div style={{
                                                                ...
                                                                background: i === 0 ? '#16a34a' : i === selRouteStops.length - 1 ? '#dc2626' : r.color,
                                                            }} />
                                            ...
                                            {isActive && selRouteStops.length === 0 && (
                                                <div style={{ padding: '4px 10px 8px', fontSize: 11, color: '#9ca3af' }}>
                                                    ⚠️ Sin paradas registradas
                                                </div>
                                            )}
                                        </div>
                                    );
                            ...
                            <ImportarRutasTxt
                                axiosAuth={axiosAuth}
                                onImportSuccess={(type, text) => { showMsg(type, text); fetchRoutes(); }}
                                onPreviewRoutes={(rutas, idx) => handlePreviewRoutes(rutas, idx)}
                ...
                <div style={{ flex: 1, position: 'relative' }}>
                    ...
                    {selRoute && selRouteStops.length > 0 && !activePreviewRoute && (
                        <div style={{
                            ...
                            border: `2px solid ${selRoute.color || '#3b82f6'}`,
                            display: 'flex', alignItems: 'center', gap: 8,
                            pointerEvents: 'none',
                        }}>
                    ...
                    {activePreviewRoute && (
                        <div style={{
                            ...
                            fontSize: 12, fontWeight: 600, color: '#111827',
                            boxShadow: '0 4px 12px rgba(0,0,0,.2)',
                            border: `2px solid ${activePreviewRoute.color}`,
                            display: 'flex', alignItems: 'center', gap: 8,
                            pointerEvents: 'none',
                        }}>
                    ...
                    >
                        ...
                        {selRoute && selRouteStops.length > 1 && (
                            <>
                                <Polyline
                                    positions={selRouteStops.map(s => [s.lat, s.lng])}
                                    pathOptions={{ color: selRoute.color || '#3b82f6', weight: 5, opacity: 0.9 }}
                                />
                                {selRouteStops.map((stop, i) => (
                                    <CircleMarker
                                        key={`sel-stop-${i}`}
                                        center={[stop.lat, stop.lng]}
                                        radius={i === 0 || i === selRouteStops.length - 1 ? 11 : 7}
                                        pathOptions={{
                                            color: i === 0 ? '#16a34a' : i === selRouteStops.length - 1 ? '#dc2626' : selRoute.color || '#3b82f6',
                                            fillColor: '#fff', fillOpacity: 1, weight: 2.5,
                                        }}
                                    >
                                        <Popup>
                                            <strong style={{ fontSize: 13 }}>
                                                {i === 0 ? '🟢 ' : i === selRouteStops.length - 1 ? '🔴 ' : `${i + 1}. `}
                                                {stop.nombre}
                                            </strong>
                        ...
                        {activePreviewRoute && (
                            <>
                                <Polyline
                                    positions={activePreviewRoute.stops.map(s => [s.lat, s.lng])}
                                    pathOptions={{ color: activePreviewRoute.color, weight: 4, opacity: 0.75, dashArray: '8 6' }}
                                />
                                {activePreviewRoute.stops.map((stop, i) => (
                                    <CircleMarker
                                        key={`preview-stop-${i}`}
                                        center={[stop.lat, stop.lng]}
                                        radius={i === 0 || i === activePreviewRoute.stops.length - 1 ? 10 : 7}
                                        pathOptions={{
                                            color: i === 0 ? '#16a34a' : i === activePreviewRoute.stops.length - 1 ? '#dc2626' : activePreviewRoute.color,
                                            fillColor: '#fff', fillOpacity: 1, weight: 2.5,
                                        }}
                                    >
                                        <Popup>
                                            <strong style={{ fontSize: 13 }}>
                                                {i === 0 ? '🟢 ' : i === activePreviewRoute.stops.length - 1 ? '🔴 ' : `${i + 1}. `}
                                                {stop.nombre}
                                            </strong>
                                            <div style={{ fontSize: 11, color: '#6b7280', marginTop: 2 }}>
                                                {stop.lat.toFixed(5)}, {stop.lng.toFixed(5)}
                        ...
                        {zones.filter(z => z.isActive).map(zone => (
                            <Polygon
                                key={zone._id}
                                positions={zone.coordinates.map(c => [c.lat ?? c.latitude, c.lng ?? c.longitude])}
                                pathOptions={{ fillColor: zone.fillColor, color: zone.strokeColor, weight: 2, fillOpacity: 0.4 }}
                            >
                        ...
                        {destination && (
                            <CircleMarker center={destination} radius={10}
                                pathOptions={{ color: '#dc2626', fillColor: '#fca5a5', fillOpacity: 0.9, weight: 2.5 }}>
