import TutoriaStatusBadge from './TutoriaStatusBadge'
import { formatearFecha } from '../../utils/tutoriaUtils'

const TutoriaCard = ({ tutoria }) => (
    <div className="bg-white border border-slate-200 w-full h-auto p-5 flex flex-col gap-3 shadow-xl rounded-lg hover:shadow-2xl transition-shadow">

        <div className="flex items-start justify-between gap-2">
            <h3 className="font-bold text-blue-950 text-lg leading-tight">
                {tutoria?.titulo || 'Sin título'}
            </h3>
            <TutoriaStatusBadge estado={tutoria?.estado} />
        </div>
            ...
            <div className="ml-1 mt-2 flex flex-col gap-1">
                {tutoria?.horarios?.length > 0 ? (
                    tutoria.horarios.map((horario, index) => (
                        <div key={index}
                            className="flex items-center gap-2 bg-blue-50 border border-blue-100 rounded-md px-3 py-1 text-sm text-blue-900">
